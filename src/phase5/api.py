...
# Esto permite que phase5 extienda phase3 sin duplicar código
from src.phase3.api import (
    app,
    inference_service,
    get_inference_service,  # helper para obtener el servicio actual
)
...
def get_sentinel_service():
    """Inicializa el servicio de Sentinel Hub de forma lazy."""
    global _sentinel_service
    if _sentinel_service is None and SENTINEL_HUB_API_KEY:
        from src.phase5.sentinel_hub import SentinelHubService
        _sentinel_service = SentinelHubService(SENTINEL_HUB_API_KEY)
    return _sentinel_service


# ── Nuevo endpoint: estado de Sentinel Hub ────────────────────────────────────
@app.get("/sentinel/status", tags=["Phase 5 — Búsqueda dinámica"])
async def sentinel_status():
    """Verifica si Sentinel Hub está configurado y disponible."""
    configured = bool(SENTINEL_HUB_API_KEY)
    return {
        "configured": configured,
        "message": (
            "Sentinel Hub activo — búsqueda dinámica disponible"
            if configured else
            "SENTINEL_HUB_API_KEY no configurada"
        ),
    }


# ── Nuevo endpoint: analizar cualquier ubicación ──────────────────────────────
...
async def analyze_location(
    ...
    Flujo:
      1. Descarga imagen Sentinel-2 de las coordenadas (via Sentinel Hub)
      2. Ejecuta el modelo U-Net (mismo modelo que las 10 ciudades)
      3. Devuelve distribución LULC + imagen original + máscara

    Args:
        lat                : Latitud del centro del área
        lng                : Longitud del centro del área
        address            : Texto de la dirección (solo para mostrar)
        size_km            : Tamaño del área a analizar en km (default 10)
        max_cloud_coverage : Nubosidad máxima 0-1 (default 0.3 = 30%)
        days_back          : Buscar imágenes de los últimos N días (default 90)

    Returns:
        address, lat, lng, size_km, meta, distribucion, mask_flat,
        mask_size, original_base64
    """
    # Validar servicios
    ...
    if sentinel is None:
        raise HTTPException(
            ...
            detail=(
                ...
                "Agrega SENTINEL_HUB_API_KEY al archivo .env."
            ),
        )
    ...
    if not (-90 <= lat <= 90) or not (-180 <= lng <= 180):
        raise HTTPException(
            status_code=422,
            detail=f"Coordenadas inválidas: lat={lat}, lng={lng}"
        )
    ...
    try:
        ...
        try:
            ...
            # Fallback: generar preview desde las bandas (B02=Blue, B03=Green, B04=Red)
        ...
        # Sin imágenes disponibles para esa ubicación/fecha
        raise HTTPException(status_code=404, detail=str(e))
    except RuntimeError as e:
        # Error de Sentinel Hub
